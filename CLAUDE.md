...
- Incidents are colour-coded by category: Fire Rescue, Medical Response, Other. The category is decided server-side by `IncidentCategory` and travels on each incident as `CATEGORY`/`CATEGORY_LABEL`, so the marker colour, the filter chips, the table badge and the `/about` copy all describe one rule. Fire is matched on the **word** "fire" anywhere in the type (`Fire Response`, `Vehicle Fire`), not on a `Fire Rescue` prefix — the prefix rule dropped every `Fire Response` call into Other. Recently-closed incidents render with muted/dashed markers and a "Closed · <duration>" status badge, and can be hidden via the "Recently Closed" toggle (persisted in `localStorage`).
