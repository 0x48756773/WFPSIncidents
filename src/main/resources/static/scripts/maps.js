...
(function () {
    ...
    // CATEGORY and CATEGORY_LABEL are omitted deliberately: the server derives both from
    // INCIDENT_TYPE, which is listed, so a corrected type already moves the signature.
    ...
    function cellsHtml(incident) {
        ...
        return `<td data-label="Incident Type"><span>${escapeHtml(incident.INCIDENT_TYPE)}</span></td>`
            ...
            + `${escapeHtml(incident.CATEGORY_LABEL)}</span></td>`
